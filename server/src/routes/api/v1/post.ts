...
router.get(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const following =
        myFollowing && myFollowing.following ? myFollowing.following : [];
      ...
      if (posts.length <= 0 && offset === 0) {
        return next(
          new ErrorHandler(404, `${username} hasn't posted anything yet.`)
        );
      ...
      const uPosts = posts.map((post) => {
        ...
        const isOwnPost =
          post._author_id.toString() === req.user._id.toString();
...
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const fetchedPost = await Post.findByIdAndUpdate(post_id, query, {
        new: true,
      });
    ...
    try {
      ...
      if (req.user._id.toString() === post._author_id.toString()) {
        ...
        res
          .status(200)
          .send(
            makeResponseJson({ ...updatedPost.toObject(), isOwnPost: true })
          );
      } else {
...
router.get(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const post = await Post.findById(Types.ObjectId(post_id)).populate({
        path: "likes",
        select: "profilePicture username fullname",
        options: {
          skip,
