...
import { BiComment, BiGlobe, BiLockAlt, BiUser } from "react-icons/bi";
...
const PostItem: React.FC<IProps> = (props) => {
  ...
  return (
    <div className="flex flex-col bg-white rounded-lg my-4 p-4 first:mt-0 dark:bg-indigo-1000">
      {/* --- AVATAR AND OPTIONS */}
            ...
            <div className="flex items-center space-x-1">
              ...
              >
                {post.privacy === "private" ? (
                  <BiLockAlt className="text-xs text-gray-500 dark:text-white" />
                ) : post.privacy === "follower" ? (
                  <BiUser className="text-xs text-gray-500 dark:text-white" />
                ) : (
                  <BiGlobe className="text-xs text-gray-500 dark:text-white" />
                )}
              </div>
            </div>
          </div>
      ...
      {isAuth ? (
        <div className="flex items-center justify-around py-2 border-t border-gray-200 dark:border-gray-800">
          ...
          <span
            ...
            onClick={handleToggleComment}
          >
            <BiComment style={{ marginRight: "8px" }} />
            Comment
          </span>
        </div>
      ) : (
