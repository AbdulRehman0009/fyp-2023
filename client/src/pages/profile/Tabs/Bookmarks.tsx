...
const Bookmarks: React.FC<IProps> = ({ username, isOwnProfile }) => {
  ...
  useDocumentTitle(`Bookmarks - ${username} | Socialo`);
  useEffect(() => {
    fetchBookmarks();
  ...
  return !isOwnProfile && username ? (
    ...
    <div className="flex flex-col items-start justify-start w-full min-h-10rem">
      ...
      {bookmarks.items.length !== 0 && !error && (
        ...
        >
          <h4 className="text-gray-700 dark:text-white mb-4 ml-4 mt-4 laptop:mt-0">
            Bookmarks
          </h4>
          <TransitionGroup component={null}>
            {bookmarks.items.map((item) => (
              <CSSTransition timeout={500} classNames="fade" key={item.id}>
                <div
                  key={item.id}
                  className="h-24 flex justify-between bg-white dark:bg-indigo-1000 rounded-md overflow-hidden"
                >
                  <div className="flex justify-center items-center">
                    <BookmarkButton
                      postID={item.post.id}
                      initBookmarkState={item.isBookmarked}
                    >
                  ...
                  >
                    <div className="flex-grow p-2 pb-4 max-w-sm">
                      ...
                      <span className="text-xs text-gray-400 self-end">
                        Bookmarked {dayjs(item.createdAt).fromNow()}
                      </span>
                    </div>
                    <div
                      className="w-32 h-full !bg-cover !bg-no-repeat !bg-center"
                      style={{
                        background: `url(${item.post.photos[0]})`,
                      }}
                    />
                  </Link>
                </div>
